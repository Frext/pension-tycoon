...
namespace _Project.Scripts.ScriptableObjects.EmployeeDict
{
    [CreateAssetMenu(menuName = "ScriptableObjects/" + nameof(EmployeeDictSo))]
    public class EmployeeDictSo : ScriptableObject
    {
        ...
        public readonly Dictionary<EmployeeTypesEnum, List<Gameplay.NPC.Employee>> employeeScriptsDict = new();
        ...
        private void SetToDefaultEmployeeScriptsDict()
        {
            ...
            for (int index = 0; index < Enum.GetNames(typeof(EmployeeTypesEnum)).Length; index++)
            {
                employeeScriptsDict.Add((EmployeeTypesEnum)index, new List<Gameplay.NPC.Employee>());
            }
        }
